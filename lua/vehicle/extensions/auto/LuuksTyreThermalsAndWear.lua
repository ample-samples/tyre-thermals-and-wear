...
local TEMP_CHANGE_RATE_CORE_FROM_SKIN = 2.7  -- Modifier for how much the temperature of the skin changes the temperature of the core
...
local function TempRingsToAvgTemp(temps, loadBias)
    local weights = tyre_utils.CalcBiasWeights(loadBias)
...
local function CalcTyreWear(dt, wheelID, groundModel, loadBias, treadCoef, slipEnergy, propulsionTorque, brakeTorque,
    ...
    for i = 1, 3 do
        ...
        tempGain = tempGain * (math.max(groundModel.staticFrictionCoefficient - 0.5, 0.1) * 2)
            ...
            + 0.15 * math.sqrt(relative_work) * TEMP_WORK_GAIN_RATE / (1 + slipEnergy ^ 2)) / tyreWidthCoeff)
        ...
        local tempDiff = (avgTemp - data.temp[i]) * 0.05
    ...
    -- TODO: scale with brake disc size, or energy lost from brake disc (if this is accessible)
...
local function updateGFX(dt)
    ...
    for i = 0, #wheels.wheelRotators do
        ...
        if wheel then
            ...
            local loadBias = loadBiasSide * 0.03 + 0.5 * (wheelCache[i].camber / 12) * wheelCache[i].wheelDir
