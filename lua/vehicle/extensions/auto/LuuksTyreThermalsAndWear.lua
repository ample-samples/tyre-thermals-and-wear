...
local TEMP_COOL_RATE = 8.0                   -- Modifier for how fast temperature cools down related to ENV_TEMP
local TEMP_COOL_VEL_RATE = 0.2                -- Modifier for how much velocity influences cooling skin
local DEFORMATION_ENERGY_MULTIPLIER = 0.1
...
local TEMP_CHANGE_RATE_CORE_FROM_SKIN = 2
local TEMP_CHANGE_RATE_CORE = 0.005      -- Modifier for how fast the core temperature changes 
local TEMP_GAIN_RATE_CORE = 0.6                -- Modifier for how fast core temperature rises from brake temp
local CORE_TEMP_VEL_COOL_RATE = 0.3               -- Modifier for how fast core temperature cools down from moving air
...
local function RecalcTyreWear(dt, wheelID, groundModel, loadBias, treadCoef, slipEnergy, propulsionTorque, brakeTorque,
    ...
    for i = 1, 3 do
        ...
        + (0.01 / (1 + slipEnergy^2))) * TEMP_GAIN_RATE
...
local function updateGFX(dt)
    ...
    for i, wd in pairs(wheels.wheelRotators) do
        ...
        local surfaceNormal = mapmgr.surfaceNormalBelow(obj:getPosition() + localVectNode2 - wd.radius*vectorUp:normalized(), 0.1)
