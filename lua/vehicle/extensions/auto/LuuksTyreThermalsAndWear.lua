...
local TEMP_SLIP_GAIN_RATE = 1.8              --
local TEMP_COOL_RATE = 5.0                  -- Modifier for how fast temperature cools down related to ENV_TEMP
...
local TORQUE_ENERGY_MULTIPLIER = 1.8       -- Modifier for how much energy is generated by torque and braking

local TEMP_CHANGE_RATE_SKIN_FROM_CORE = 0.1 -- Modifier for how fast skin temperature changes from core
...
local function CalcTyreWear(dt, wheelID, groundModel, loadBias, treadCoef, slipEnergy, propulsionTorque, brakeTorque,
    ...
    for i = 1, 3 do
        ...
        data.temp[i] = data.temp[i] +
             dt * (tempGain - (tempCoolingRate * coolVelCoeff + staticCoolingRate) + tempDiff + skinTempDiffCore) * TEMP_CHANGE_RATE /
            tyreWidthCoeff
    end
