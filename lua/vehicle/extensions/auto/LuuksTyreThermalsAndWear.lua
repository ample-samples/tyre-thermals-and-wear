...
local TEMP_SLIP_GAIN_RATE = 2.3              --
local TEMP_COOL_RATE = 0.4                   -- Modifier for how fast temperature cools down related to ENV_TEMP
local TEMP_COOL_VEL_RATE = 0.075               -- Modifier for how much velocity influences cooling skin
local DEFORMATION_ENERGY_MULTIPLIER = 0.15
local TORQUE_ENERGY_MULTIPLIER = 1.6         -- Modifier for how much energy is generated by torque and braking
...
local function CalcTyreWear(dt, wheelID, groundModel, loadBias, treadCoef, slipEnergy, propulsionTorque, brakeTorque,
    ...
    for i = 1, 3 do
        ...
        data.temp[i] = data.temp[i] +
            dt * (tempGain - (tempCoolingRate + tempCoolingRate * velCoolingCoeff + staticCoolingRate) + tempDiff + skinTempDiffCore) *
            TEMP_CHANGE_RATE /
            tyreWidthCoeff
    end
    ...
    wheelCache[wheelID].brakeCooling = (((brake_coolingSetting or 4) - 1) / 3) * baseBrakeCoolings[wheelID]
    -- data.condition = 100
    ...
    for i = 0, #wheels.wheelRotators do
        ...
        if wheel then
            ...
            if not degubStepFinished then
                ...
                for key, value in pairs(wheels.wheelRotators[i]) do
                    wheelRotatorsKeys[key] = "0"
                end
                htmlTools.dumpToFile(wheelRotatorsKeys, "wheel")
