...
local TEMP_WORK_GAIN_RATE = 7.5              -- Modifier for how fast temp rises if wheelSpin = FALSE // Modifier for how fast temperature rises from wheel side loading e.i. generating G's
local TEMP_SLIP_GAIN_RATE = 30.55            -- Modifier for how fast temp rises if wheelSpin = TRUE // Should be 1.7X!!!
...
local TEMP_CHANGE_RATE_CORE_FROM_SKIN = 1.7  -- Modifier for how much the temperature of the skin changes the temperature of the core
...
local function CalcTyreWear(dt, wheelID, groundModel, loadBias, treadCoef, slipEnergy, propulsionTorque, brakeTorque,
    ...
    for i = 1, 3 do
        ...
        tempGain = tempGain * (math.max(groundModel.staticFrictionCoefficient - 0.5, 0.1) * 2)
            ...
            + ((((0.004 * slipEnergy + 0.005 * (slipEnergy / 2) ^ 2) * loadCoeffIndividual * 1400) * TEMP_SLIP_GAIN_RATE
                -- Temp gain from work done in corner
                -- TODO: Test horizontal G * weights[i]

                + wheelCache[wheelID].wheelDir * loadBias * 0.2 * relative_work * TEMP_WORK_GAIN_RATE / (1 + slipEnergy ^ 2)) * groundModel.staticFrictionCoefficient / tyreWidthCoeff)
...
local function updateGFX(dt)
    ...
    for i = 0, #wheels.wheelRotators do
        ...
        if wheel then
            ...
            for i = 1, #wheels.wheelRotators[0].nodes, 1 do
                v.data.nodes[wheels.wheelRotators[0].nodes[i]].stribeckExponent = 5
            end
            dump("================= obj funcs")
