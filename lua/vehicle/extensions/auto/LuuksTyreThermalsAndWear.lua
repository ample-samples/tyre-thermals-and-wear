...
local TEMP_CHANGE_RATE = 0.80                 -- Global modifier for how fast skin temperature changes
local TEMP_WORK_GAIN_RATE = 2                    -- Modifier for how fast temperature rises from wheel load e.i. generating G's
local TEMP_SLIP_GAIN_RATE = 1               -- 
local TEMP_COOL_RATE = 17.0                   -- Modifier for how fast temperature cools down related to ENV_TEMP
local TEMP_COOL_VEL_RATE = 0.05                -- Modifier for how much velocity influences cooling skin
local DEFORMATION_ENERGY_MULTIPLIER = 0.02
local TORQUE_ENERGY_MULTIPLIER = 0.025         -- Modifier for how much energy is generated by torque and braking

local TEMP_CHANGE_RATE_SKIN_FROM_CORE = 0.08 -- Modifier for how fast skin temperature changes from core

local TEMP_CHANGE_RATE_CORE_FROM_SKIN = 3
...
local function CalcBiasWeights(loadBias)
    ...
    local weightLeft = -loadBias + 1
    local weightRight = loadBias + 1
...
local function RecalcTyreWear(dt, wheelID, groundModel, loadBias, treadCoef, slipEnergy, propulsionTorque, brakeTorque,
    ...
    local netTorqueEnergy = math.abs(propulsionTorque * 0.015 - brakeTorque * 0.015) * 0.01*angularVel * TORQUE_ENERGY_MULTIPLIER * 3
    ...
    for i = 1, 3 do
        ...
        local tempGain = (slipEnergy * 0.3 + netTorqueEnergy * 0.05 + deformationEnergy * 0.001 * DEFORMATION_ENERGY_MULTIPLIER) * 3 * weights[i]
        tempGain = tempGain * (math.max(groundModel.staticFrictionCoefficient - 0.5, 0.1) * 2)
        -- Temp gain from wheelspin / lockup
        + ((0.001 * slipEnergy * loadCoeffIndividual) * TEMP_SLIP_GAIN_RATE
        -- Temp gain from work done in corner
        + (0.01 / (1 + slipEnergy^2)) * 3 * weights[i]) * TEMP_WORK_GAIN_RATE
        ...
        local tempDiff = (avgTemp - data.temp[i])
...
local function updateGFX(dt)
    ...
    for i, wd in pairs(wheels.wheelRotators) do
        ...
        -- dump("wname " .. w.name)
        -- dump("relative camber " .. wheelToBodyCamber)
        -- dump("absolute toe vector to worldspace:")
        -- dump(vectorWheelForward)
        -- dump("absolute caster vector to worldspace:")
        -- dump(vectorWheelUp)
