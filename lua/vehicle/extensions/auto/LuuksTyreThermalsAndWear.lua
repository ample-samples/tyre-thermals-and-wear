...
local function CalcTyreWear(dt, wheelID, groundModel, loadBias, treadCoef, slipEnergy, propulsionTorque, brakeTorque,
    ...
    for i = 1, 3 do
        ...
        local skinTempDiffCore = (data.temp[7] - avgTemp) * TEMP_CHANGE_RATE_SKIN_FROM_CORE 
    ...
    local coreTempDiffSkin = (avgSkin - data.temp[7]) * TEMP_CHANGE_RATE_CORE_FROM_SKIN * (0.5 * treadCoef)
    ...
    (coreTempDiffSkin + coreTempDiffBrake * 0.7 - coreTempCooling) * TEMP_CHANGE_RATE_CORE * dt / tyreWidthCoeff

    local thermalCoeff = (math.abs(avgTemp - data.working_temp) / data.working_temp) ^ 0.8

    local wear = tempDistToWearMult(tempDistAverage) *
        (slipEnergy * 1.75 * lerp(1, 1.1, math.max(0, tempDistAverage-10)) + (vehNotParked * math.abs(propulsionTorque * 0.008 - brakeTorque * 0.025) * 0.3 *
