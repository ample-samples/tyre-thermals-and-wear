...
local TEMP_WORK_GAIN_RATE = 1.5              -- Modifier for how fast temp rises if wheelSpin = FALSE // Modifier for how fast temperature rises from wheel side loading e.i. generating G's
local TEMP_SLIP_GAIN_RATE = 3.0
local TEMP_COOL_RATE = 0.6                   -- Modifier for how fast temperature cools down related to ENV_TEMP
local TEMP_COOL_VEL_RATE = 0.14              -- Modifier for how much velocity influences cooling skin
local DEFORMATION_ENERGY_MULTIPLIER = 0.25
local TORQUE_ENERGY_MULTIPLIER = 0.0         -- Modifier for how much energy is generated by torque and braking // Rotational inertia?

local TEMP_CHANGE_RATE_SKIN_FROM_CORE = 0.15 -- Modifier for how fast skin temperature changes from core

local TEMP_CHANGE_RATE_CORE_FROM_SKIN = 0.7  -- Modifier for how much the temperature of the skin changes the temperature of the core
...
local function CalcTyreWear(dt, wheelID, groundModel, loadBias, treadCoef, slipEnergy, propulsionTorque, brakeTorque,
    ...
    local weights = CalcBiasWeights(loadBias)
    local avgWeightedTemp = TempRingsToAvgTemp(data.temp, loadBias)
    ...
    local tempDistWeighted = avgWeightedTemp / data.working_temp

    for i = 1, 3 do
        local loadCoeffIndividual = weights[i] * load
        local tempLerpValue = (tempDistWeighted / data.working_temp) ^ 0.8
        ...
        tempGain = tempGain * (math.max(groundModel.staticFrictionCoefficient - 0.5, 0.1) * 2)
            ...
            + (((0.009 * slipEnergy ^ 2 * loadCoeffIndividual) * TEMP_SLIP_GAIN_RATE
        ...
        local velCoolingCoeff = math.sqrt(airspeed) * TEMP_COOL_VEL_RATE / treadCoef * dt
        local skinTempDiffCore = (data.temp[4] - avgWeightedTemp) * TEMP_CHANGE_RATE_SKIN_FROM_CORE /
            (0.5 * treadCoef) -- Slicks hold heat better

        local tempDiff = (avgWeightedTemp - data.temp[i]) * 0.2
    ...
    local thermalCoeff = (math.abs(avgWeightedTemp - data.working_temp) / data.working_temp) ^ 0.8
    local loadRaw = loadRaw or vehicleMass
    local vehicleWeightNewtowns = vehicleMass * 9.81

    local wear = tempDistToWearMult(tempDistWeighted)
        * (loadRaw / vehicleWeightNewtowns * slipEnergy * lerp(1, 20.0, math.max(1, tempDistWeighted)) / tyreWidthCoeff
            + (vehNotParked * math.abs(propulsionTorque * 0.008 - brakeTorque * 0.025) * 0.3 * TORQUE_ENERGY_MULTIPLIER)
            * 0.08 + angularVel * 0.05)
        * WEAR_RATE * dt 
...
local function CalculateTyreGrip(wheelID, loadBias, treadCoef)
    ...
    tyreGrip = tyreGrip * (tyre_data.tempToGrip.slicks[math.floor(avgTemp)] or 0.97)
    ...
    for i = 0, #wheels.wheelRotators do
        ...
        if wheel then
            ...
            CalcTyreWear(dt, i, groundModel, loadBias, treadCoef, slipEnergy, propulsionTorque, brakeTorque, load,
                ...
                wheelCache[i].name, wheelCache[i].downForceRaw)
